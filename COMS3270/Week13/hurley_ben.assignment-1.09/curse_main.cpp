...
void battle_handler(character *pc, character *npc, int is_wild, int debug) {
    ...
    if (debug) {
        const char* line4 = is_wild ? "Debug: Press ENTER to view wild stats" : "Debug: Press ENTER to view starter stats";
    ...
    int needs_redraw = 0;
    int pc_pokechoice = 0;
    int npc_pokechoice = 0;
    int pc_pokedefeat = 0;
    int npc_pokedefeat = 1;
    int pc_totalhealth = pc->pokemon_party[pc_pokechoice]->current_hp;
    int npc_totalhealth = npc->pokemon_party[pc_pokechoice]->current_hp;
    ...
    while(!battle_won) {
        ...
        if(ch == KEY_ENTER || ch == 10) {
            ...
            char line_intro[78];
            if(pc_pokedefeat) {
                // ask player what pokemon they would like to use, then update pc_totalhealth
            }
            if(npc_pokedefeat) {
                if(!is_wild) {
                    sprintf(line_intro, "%s is going to use %s!", npc->charName, npc->pokemon_party[0]->base_species->name.c_str());
                } else {
                    sprintf(line_intro, "%s is going to attack!", npc->pokemon_party[0]->base_species->name.c_str());
                }
                // do "AI" logic in which a random new pokemon is chosen, then update npc_totalhealth
                npc_pokedefeat = 0;
            }
            ...
            box(npc_box, 0, 0);
            box(choice_box, 0,0);
            mvwprintw(choice_box, 3, (78 - strlen(line_intro)) / 2, "%s", line_intro);
            mvwprintw(pc_box, 1, 1, "%s",pc->pokemon_party[pc_pokechoice]->base_species->name.c_str());
            mvwprintw(npc_box, 1, 1, "%s",npc->pokemon_party[npc_pokechoice]->base_species->name.c_str());
            mvwprintw(pc_box, 2, 1, "HP: %d/%d",pc->pokemon_party[pc_pokechoice]->current_hp, pc_totalhealth);
            mvwprintw(npc_box, 2, 1, "HP: %d/%d",npc->pokemon_party[npc_pokechoice]->current_hp, npc_totalhealth);
