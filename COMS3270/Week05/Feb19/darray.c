...
int darray_num_elements(darray *d, int *elements){
    *elements = (*d)->elements;
    return 0;
    // the parentheses are important, as it dereferences d then finds the elements, *d->elements would dereference elements then look for it, which would crash.
}

int darray_capacity(darray *d, int *capacity){
    *capacity = (*d)->capacity;
    return 0;
}

int darray_remove(darray *d, int index, void **v){
    if(index < 0 || index >= (*d)->elements) {
        return -1;
    }

    memcpy(v, (*d)->a + (index * (*d)->element_size), (*d)->element_size);

    memmove((*d)->a + (index * (*d)->element_size), (*d)->a + ((index + 1) * (*d)->element_size), ((*d)->elements - index - 1) * (*d)->element_size);
