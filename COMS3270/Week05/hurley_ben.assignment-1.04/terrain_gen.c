    ...
    for(int i = 0; i < ROW; i++) {
        for(int j = 0; j < COLUMN; j++) {
            ...
            if (m->characters[i][j] != NULL) {
                ...
                printf("%c", m->characters[i][j]->type);
            }
...
void move_npc(character *c, map *m) {
    ...
    if ((c->type == PACER || c->type == WANDERER || c->type == EXPLORER || c->type == PC || c->type == SWIMMER) && 
        c->dir_x == 0 && c->dir_y == 0) {
        int dir_idx = rand() % 8;
    ...
    switch(c->type) {
        ...
        break; 
        case PC:
        // for testing purposes, the PC uses explorer logic to move (I thought about letting player control movement, but I'm lazy)
            new_x = c->x + c->dir_x;
            new_y = c->y + c->dir_y;
